-- V1 baseline: the 16-table start state quantcore.db.py already assumes.
--
-- This is the start state db/flyway.conf's `baselineVersion=1` refers to but
-- never wrote down (see issue #165). Extracted VERBATIM from
-- quantcore/db.py's `_SCHEMA` string as it stood immediately before
-- V2__positions_multi_owner.sql was added -- i.e. `git show 9a9d204^:quantcore/db.py`,
-- commit dated 2026-06-13.
--
-- NOT under db/migrations/ and NOT read by flyway.locations (db/flyway.conf) --
-- a version at or below the baseline is ignored or rejected by Flyway. This
-- file exists only as a reference start state for building a database from
-- empty (used by tests/schema tooling, see docs/proposals/schema-ownership-plan.md).
--
-- FROZEN. Schema changes go in db/migrations/V*.sql (V2 onward), never here.
