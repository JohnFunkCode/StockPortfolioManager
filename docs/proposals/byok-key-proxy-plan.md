...
**Runbook results — prod leg (2026-07-18):**

- *Test-leg close:* John's real-key browser E2E on test quantui succeeded 2026-07-18 ("everything
  is working properly"), green-lighting the prod leg.
- *Identity secrets:* `quantui-signing-key` generated by John and piped straight into Secret
  Manager (a gcloud internal retry surfaced a spurious "already exists" conflict; the version-1
  timestamps match his run, confirming the stored key IS the fresh one). Public half derived
  in-place (`secrets versions access | openssl pkey -pubout`) → `quantui-signing-pub` v1.
  `keyproxy-runtime@quantcore-prod-20260606` SA created with zero project-level roles; per-secret
  `secretAccessor` grants: `keyproxy-runtime@` on `keyproxy-private-key` + `quantui-signing-pub`,
  `quantcore-run@` on `quantui-signing-key` + `quantui-signing-pub`.
- *Image:* keyproxy `177e411` copied test→prod AR by digest (`sha256:9b3b0ecb…`) via
  `docker buildx imagetools create` — same mechanism `prod-rollout.yml` uses.
- *Deploy:* `quantcore-keyproxy` rev `00001-rlv` IAM-locked (`--no-allow-unauthenticated`,
  `--max-instances=1`, `--timeout=300`, dedicated runtime SA), URL
...
- *Incident (resolved same session):* the first env-wiring commands used `--set-secrets`, which
  **replaces** a service's whole secret set rather than merging. quantui briefly lost
  ...
  `QUANTCORE_DB_DSN` and failed startup — Cloud Run kept traffic on the last healthy revision, so
  the api never went down. Repaired with `--update-secrets` merges; final revision envs verified
  as strict supersets of the last known-good configs, and both auth gates re-verified
  (api unauthenticated → 401, quantui → IAP 302). **Lesson: on existing services always use
