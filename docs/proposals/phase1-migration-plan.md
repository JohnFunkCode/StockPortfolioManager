...
### Resume after restart (next: Step 7 — Recommendations)

Last DONE: **Step 6** (branch `feature/new-architecture-phase1`; commit + push pending — user commits). To resume on any machine:

1. `git checkout feature/new-architecture-phase1 && git pull`
2. **Restart the test-DB proxy** (it does not survive a reboot) — Step 7 runs the suite against the test DB, never prod:
...
4. Continue at **Step 7 (Recommendations)**: `RecommendationsService` composes Prices/Options/Microstructure/Sentiment/Fundamentals service *instances* (constructor injection in `registry.py`); absorbs `get_trade_recommendation` (19 signals), `get_stop_loss_analysis`, `get_relative_strength(+history)` — replacing today's fragile cross-server imports of decorated tool fns in `stock_price_server.py` (which then shrinks to ~400 lines of thin adapters). Test via injected fake services returning canned dicts (assert scoring outcomes), per the testing strategy.
