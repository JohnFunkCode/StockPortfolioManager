...
### Resume after restart (next: Step 9 — Fix collect_options.py in place)

Last DONE: **Step 8** (branch `feature/new-architecture-phase1`; commit + push pending — user commits). To resume on any machine:

1. `git checkout feature/new-architecture-phase1 && git pull`
2. **Restart the test-DB proxy** (it does not survive a reboot) — Step 9 runs the suite against the test DB, never prod:
...
4. Continue at **Step 9 (Fix `collect_options.py` in place)**: rewrite its broken imports against `OptionsService` (snapshot collection = `refresh_snapshots`/`fetch_and_store_full_chain` paths via `get_services().options`), keep its argparse surface, drop the `--db` sqlite default. Step 8 (options screening split) is DONE: `options_analysis.py` is now a thin adapter and no longer imports `news_collector`/`options_contract_tools`/`options_store`/`news_store` — so all four shims are deletable in Step 10 once `collect_options.py` is repointed.
   - **SURFACE on prod promotion:** (a) Step 3 commit `08b725f` fixed a pre-existing PROD bug (`cache_get_all_latest` SQLite-style GROUP BY rejected by Postgres). (b) Step 5 backfill 404 path now single-appends `no_data` (was double-append). (c) Step 6 V2 migration (`db/migrations/V2__positions_multi_owner.sql`) relaxes legacy `positions` NOT NULL/CHECK constraints + adds owner/CSV-parity cols — **must be applied to prod via Flyway before prod use**; the one-time `scripts/import_portfolio.py --csv portfolio.csv --owner john --allow-prod` import is a deliberate user-initiated step.
...
| 5 | Options service + PolygonGateway | DONE | — | 2026-06-13 | quantcore/gateways/polygon_gateway.py (PolygonGateway: .has_key + option_snapshots() w/ next_url pagination, PolygonPlanError on 403, None on 404) + quantcore/analytics/options_math.py (PURE: bs_d1/bs_delta/bs_gamma/chain_side_full/compute_max_pain/compute_expected_move/safe_int — single home, kills _bs_delta ×2, max-pain, expected-move, _chain_side_full ×2). quantcore/services/options.py: OptionsService absorbs 6 stock_price options tools (get_full_options_chain, get_option_contracts, price_vertical_spread, get_unusual_calls, get_delta_adjusted_oi, get_gamma_wall_history) + 10 REST surfaces (options latest/history/analytics/chain/iv-rank, signals/options-flow, portfolio/delta-exposure, history/backfill, refresh-options-snapshots). backfill_options_history returns (payload, http_status) so the thin route preserves 400/402/200 codes; gateway's None-on-404 fixes the original route's double-append-no_data quirk (now single-append — SURFACE on prod promotion). refresh_options_snapshots composes PricesService (atm path = prices.get_stock_price; full path = self.get_full_options_chain) — registry constructs prices FIRST so the dependency is acyclic. PricesService.get_risk_signals added (drawdown + vwap) for /signals/risk. stock_price_server.py: 6 options tools now thin adapters (docstrings verbatim); dead helpers removed (_safe_int/_summarize_options/_chain_side_full/_norm_cdf/_norm_pdf/_bs_d1/_bs_gamma/_bs_delta + orphaned import datetime/options_store/options_contract_tools/_options_store). api/app.py: all 10 options routes one call deep; _compute_max_pain/_bs_delta_local/_compute_expected_move deleted. Dedup verified: bs_delta/max_pain/chain_side_full/expected_move ×1 each (all in options_math.py). 41 tests green vs test DB; 5 MCP servers boot (stock-price 23 tools); Flask 37 routes |
| 6 | Harvester + Portfolio (DB positions, multi-owner) | DONE | — | 2026-06-13 | quantcore/repositories/harvester_repository.py: HarvesterPlanDB + PlanBuildParams + all SQL_* constants + fetch_daily_history_ohlcv moved verbatim from experiments/HarvesterPlanStore.py (SQL only). quantcore/services/harvester.py: HarvesterService wraps the repo (passthrough CRUD/queries) and absorbs the former HarvesterController behaviour verbatim (get_next_actions/scan_and_fire_alerts/record_execution). PORTFOLIO → DB: db/migrations/V2__positions_multi_owner.sql (Flyway) adds owner TEXT NOT NULL DEFAULT 'john' + CSV-parity cols (name/purchase_price/quantity/purchase_date/currency/sale_price/sale_date), relaxes legacy cols (DROP NOT NULL on opened_at/entry_price/shares/cost_basis_total + DROP the three CHECK(>0/>=0) constraints — REST add path allows positions without price/qty), unique idx_positions_owner_symbol_date(owner,symbol_id,purchase_date); mirrored into quantcore/db.py _SCHEMA (additive, IF NOT EXISTS). quantcore/repositories/portfolio_repository.py NEW: owner-scoped CRUD, replace_owner_positions(owner,rows) single-txn full-sync, _legacy_values() derives the nullable legacy cols from CSV fields, _row_to_dict() returns the exact CSV-parity shape (source="portfolio", tags=[]) the WebUI expects. quantcore/services/portfolio.py NEW: PortfolioService (import_csv full-replace, list_positions(owner="john"), add_position raising DuplicateSymbolError→409, remove_position, list_owners) + DuplicateSymbolError. registry wires harvester_repository/portfolio_repository + harvester/portfolio services. scripts/import_portfolio.py NEW (--csv/--owner/--allow-prod, assert_not_production guard). CALLERS repointed: api/app.py (Harvester routes + portfolio GET/POST/DELETE now DB-backed via service with ?owner= default john; NEW POST /api/portfolio/import multipart-or-path → 38 routes), notifier.py (get_services().harvester), main.py (Portfolio built from portfolio.list_positions("john") via new Portfolio.read_stocks_from_records). experiments/HarvesterPlanStore.py → re-export shim (HarvesterController(db)→HarvesterService(repo=db); delete in Step 10). TESTS: test_portfolio_service.py (9: import count, skip-empty-symbol, full-sync replace, re-import idempotency, owner isolation, add+dup-raise, no-symbol ValueError, remove count, remove-missing zero) + test_harvester_service.py (8: seeds template/instance/rungs directly — offline, no yfinance — covers get_plan/rungs, status filter, harvest_hit_for_symbol price thresholds, mark_rungs_achieved PENDING-guard, get_next_actions advance, record_execution, delete→SUPERSEDED, alerts). 58 tests green vs test DB (DISCORD/BUCKET unset); 5 MCP servers boot (47 tools total); Flask 38 routes; registry wires 17 fields incl. harvester+portfolio. SURFACE ON PROD PROMOTION: V2 migration must be applied to prod via Flyway before prod use, and the one-time `scripts/import_portfolio.py --csv portfolio.csv --owner john --allow-prod` import is a deliberate user-initiated step |
| 7 | Recommendations service | DONE | — | 2026-06-13 | quantcore/services/recommendations.py NEW: RecommendationsService — the one service that composes other *service instances* (Prices/Options/Microstructure/Sentiment/Fundamentals) + OhlcvRepository + YFinanceGateway via constructor injection. Absorbs verbatim from stock_price_server.py: get_stop_loss_analysis (price/Bollinger/VWAP/MACD/RSI + gamma-wall + drawdown + short-interest synthesis), get_relative_strength (1/3/6/12m vs SPY/QQQ/sector ETF, live yf.download), get_relative_strength_history (daily RS from cached OHLCV), get_trade_recommendation (19-signal engine). _SECTOR_ETF_MAP + _get_sector_etf moved here (RS is their only consumer). Cross-server fragility ELIMINATED: the four tools previously imported each other's decorated tool fns (get_stock_price/get_vwap/get_macd/get_rsi/get_delta_adjusted_oi/get_unusual_calls/microstructure+fundamentals+sentiment) — now plain method calls on injected instances. YFinanceGateway.download() added (bulk multi-ticker, progress suppressed) for RS live fetch; get_relative_strength_history keeps the cached-OHLCV path via OhlcvRepository — both data paths preserved for parity. registry hoists options/microstructure/sentiment/fundamentals to shared locals so RecommendationsService composes the live instances (verified: r._prices is svc.prices etc.). stock_price_server.py: 4 remaining inline-logic tools → thin adapters (docstrings verbatim); file 1730→703 lines; orphaned imports removed (math, yfinance, ohlcv_cache get_history/period_to_days). Grep confirmed NO external importer of the moved fns/helpers. TESTS: test_recommendations_service.py (7, offline — inject fake services returning canned dicts: strong-bull→LONG_CALL/HIGH, strong-bear→LONG_PUT, neutral→SKIP, missing-price→SKIP+error, HIGH-SI squeeze override→LONG_CALL, imminent-earnings override→SKIP, stop-loss places technical stop below price w/ VWAP primary support). 65 tests green vs test DB (DISCORD/BUCKET unset); 5 MCP servers boot (47 tools, stock-price 23); Flask boots (create_app, /api/health 200); registry wires 18 fields incl. recommendations composing shared instances |
| 8 | Options screening split | DONE | — | 2026-06-13 | quantcore/services/options_screening.py NEW: OptionsScreeningService absorbs ALL analytics verbatim from fastMCPTest/options_analysis.py — config constants (BB/PC/IV-rank/ROI/guardrail), 5 dataclasses (BollingerBands/PutCallAnalysis/IVAnalysis/OptionsSummary/SecurityAnalysis), pure helpers (_safe_float/_safe_int/_chain_pc), live fetchers (fetch_bollinger_bands→ohlcv repo, fetch_options/fetch_put_call_analysis/fetch_earnings_proximity/fetch_recent_positive_catalyst→YFinanceGateway expirations/option_chain/calendar/news, fetch_iv_analysis, fetch_security w/ news_store per-call param), score (19 scoring rules), build_put_trade/build_call_trade, put_guardrail_reason/call_guardrail_reason, combined_put/call_rank_score, greedy_fill, load_watchlist/is_us_listed (static), _build_candidate_summary/_run_analysis, analyze_watchlist/analyze_symbol (the 2 MCP tool bodies). Constructor: (ohlcv_repository, yfinance_gateway) only — analytics fetch live; OptionsStore persistence + the contracts/spread tools route through the registry. fastMCPTest/options_analysis.py now a THIN ADAPTER (1684→~580 lines): FastMCP server w/ 5 thin tools (mcp_health_check keeps its diag dict; analyze_options_watchlist/symbol→get_services().options_screening.*; get_option_contracts/price_vertical_spread→get_services().options.* — docstrings verbatim) + print_* presentation (calls svc=get_services().options_screening for build/guardrail/rank/greedy) + CLI main(). main() rewritten: persistence via get_services().options_repository (REMOVED the dead store.db_path/store.db_path.name prints — OptionsStore has no db_path in the Postgres era, already broken pre-refactor), news via get_services().news_repository + sentiment.collect(symbols, score=True) wrapped in graceful-degradation try/except; --db kept in argparse but deprecated (no-op warning; DSN comes from env). Adapter imports ONLY argparse/platform/sys/Path/Optional/fastmcp + quantcore.services (registry + options_screening for SecurityAnalysis dataclass + presentation constants) — no yfinance/psycopg2/repository imports (Step 10 audit pre-satisfied; news_collector/options_contract_tools/options_store/news_store imports all eliminated → those shims now deletable in Step 10). registry wires options_screening=OptionsScreeningService(ohlcv/yfinance). TESTS: test_options_screening_service.py (15, fully offline — construct SecurityAnalysis directly, inert stub collaborators: score oversold-bull=10/overbought-bear=11, news directional adjust, midband no-signal; guardrails earnings-blackout-both/catalyst-puts-only/contradiction/bearish-news-calls/clean; ranking blend + ROI cap + greedy budget/affordability; is_us_listed suffixes + load_watchlist parse). 80 tests green vs test DB (DISCORD/BUCKET unset); 5 MCP servers boot (47 tools, options-analysis 5); Flask /api/health 200; registry wires 19 fields incl. options_screening |
