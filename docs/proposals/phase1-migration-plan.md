...
### Resume after restart (next: Step 8 — Options screening split)

Last DONE: **Step 7** (branch `feature/new-architecture-phase1`; commit + push pending — user commits). To resume on any machine:

1. `git checkout feature/new-architecture-phase1 && git pull`
2. **Restart the test-DB proxy** (it does not survive a reboot) — Step 8 runs the suite against the test DB, never prod:
...
4. Continue at **Step 8 (Options screening split)**: split `fastMCPTest/options_analysis.py` — analytics (dataclasses, `fetch_security`, `score`, `build_put_trade`/`build_call_trade`, `_greedy_fill`, guardrails, `load_watchlist`, `is_us_listed`) move to `quantcore/services/options_screening.py` (`OptionsScreeningService`); the file keeps its FastMCP server (5 thin tools) + CLI `main()` + `print_*` presentation. After this, `news_collector.py` is the last remaining shim still imported by `options_analysis` (the `NewsCollector` import) — once Step 8 repoints it, that shim can be deleted in Step 10.
