...
### Resume after restart (next: Step 6 — Harvester + Portfolio)

Last DONE: **Step 5** (branch `feature/new-architecture-phase1`, pushed). To resume on any machine:

1. `git checkout feature/new-architecture-phase1 && git pull`
2. **Restart the test-DB proxy** (it does not survive a reboot) — Step 6 does DB-touching work (schema change) and MUST run against the test DB, never prod:
...
4. Continue at **Step 6 (Harvester + Portfolio)**: `HarvesterPlanDB`→`harvester_repository.py`, `HarvesterController`→`HarvesterService`; positions DB migration (Flyway `V2__positions_multi_owner.sql` + `init_schema()` mirror, owner-scoped `PortfolioRepository`/`PortfolioService`, CSV import full-sync); repoint `main.py`/`notifier.py`/`api/app.py` Harvester+portfolio callers; shim then (Step 10) delete `experiments/HarvesterPlanStore.py`. New tests: `test_harvester_service.py`, `test_portfolio_service.py`.
   - **SURFACE on prod promotion:** (a) Step 3 commit `08b725f` fixed a pre-existing PROD bug (`cache_get_all_latest` SQLite-style GROUP BY rejected by Postgres). (b) Step 5 backfill 404 path now single-appends `no_data` (was double-append).
