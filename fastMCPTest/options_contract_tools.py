...
The MCP servers expose these helpers as tools, but the business logic lives
here so stock_price_server.py and options_analysis.py stay consistent.
"""
...
def _chain_side_full(chain_df) -> dict:
    df = chain_df.copy()
    ...
    return {
        ...
        "total_open_interest": int(df["openInterest"].fillna(0).sum()),
        "total_volume": int(df["volume"].fillna(0).sum()),
        "avg_iv_pct": round(float(df["impliedVolatility"].fillna(0).mean()) * 100, 2),
    }


def fetch_and_store_full_chain(symbol: str, store: OptionsStore) -> dict:
    ...
    for exp_date in ticker.options:
        ...
        calls = _chain_side_full(chain.calls)
        puts = _chain_side_full(chain.puts)
