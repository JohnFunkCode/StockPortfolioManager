"""HarvesterPlanDB — persistence for the Harvester harvest-ladder strategy.

Moved verbatim from experiments/HarvesterPlanStore.py (Phase 1 Step 6,
architectural standard v2). Owns all plan/rung/alert SQL, the yfinance OHLCV
ingestion used to build a plan, and the dynamic-ladder build path (which calls
the pure planner in experiments/HarvesterExperiment.py). Behavior is unchanged;
only the module location moved. The HarvesterController orchestration that used
to live alongside this class now lives in quantcore.services.harvester as
HarvesterService.

All database access goes through quantcore.db.get_connection(), whose
sqlite3-compatible wrapper translates the ? / :name parameter styles below to
