...
- `CLOUDSQL_CONNECTION_NAME` / `CLOUDSQL_PORT` / `CLOUDSQL_QUOTA_PROJECT` — where `runProxy-MAC.sh` points the Cloud SQL Auth Proxy for **prod** (`<project>:<region>:<instance>`, port `5433`). Not credentials; the proxy authenticates with your ADC
- `CLOUDSQL_TEST_CONNECTION_NAME` / `CLOUDSQL_TEST_PORT` / `CLOUDSQL_TEST_QUOTA_PROJECT` — the same three for **test** (port `5434`), used by `runProxy-MAC.sh --test`. See [Connecting to a Database Locally (Mac)](#connecting-to-a-database-locally-mac)
...
## Connecting to a Database Locally (Mac)

Both QuantCore databases live in Cloud SQL and are reached through the **Cloud SQL Auth Proxy**,
which exposes the remote instance as a local `host:port`. One script starts it, and the
`--test`/`--prod` flag convention matches [`./scripts/flyway.sh`](#migrations-flyway):
...
Prod is the default because it is the system of record and because
`scripts/restart_local_stack.sh` calls this script bare and expects `:5433`. (`runUI-MAC.sh`
carries its own copy of the proxy-start logic and is likewise prod-only.) The two proxies are
independent — the guard matches on the port, so a running test proxy does not stop you starting
the prod one, and vice versa. Logs go to `cloud-sql-proxy.log` and `cloud-sql-proxy-test.log`
(both gitignored).

Connection targets come from `.env`, three keys per environment
(`CLOUDSQL_CONNECTION_NAME` / `_PORT` / `_QUOTA_PROJECT`, and the `CLOUDSQL_TEST_*` trio). Nothing
here reads a DSN or a password; `--test` tells you exactly which keys to add if they are missing.

Application code reads `QUANTCORE_DB_DSN`, which points at **prod**. To run something against the
test database instead, wrap it — this swaps in `QUANTCORE_TEST_DB_DSN` for the child process only:
...
The **test suite needs no wrapper** — `tests/__init__.py` performs the same swap itself, before
`quantcore.db` freezes its DSN at import. The wrapper is for everything else: one-off scripts,
`psql`, an ad-hoc `python -c`.

It reads only that one key out of `.env` (not the prod DSN, webhook, or API keys), and echoes the
target as `host:port/name` — never the DSN, which carries the password.

> **`psql` does not read `QUANTCORE_DB_DSN`.** Invoked as `with-test-db.sh psql -c …` it falls
> through to your local socket and quietly queries the wrong database. Pass the DSN explicitly:
> `./scripts/with-test-db.sh sh -c 'psql "$QUANTCORE_DB_DSN" -c "\d positions"'`
...
To verify a deployed wrapper, use the bounded smoke utility with one endpoint and tool per entry.
It initializes the session, optionally holds it open, calls the selected tool, and reports only
metadata rather than response payloads:

```bash
PYTHONPATH=. .venv/bin/python scripts/mcp_http_smoke.py \
  --endpoint stock-price=https://example.run.app/mcp \
