#!/usr/bin/env python3
"""Generate the legacy HTML portfolio report, and optionally publish it to S3.

This is the report that used to be a side effect of ``main.py``'s daily run
(issue #147). It is preserved here as-is -- same charts, same Jinja2 template,
same two retired sections -- but publishing is now a deliberate act rather
than something the Cloud Run Job does on the way past.

    python scripts/generate_portfolio_report.py                    # write ./portfolio_report.html
    python scripts/generate_portfolio_report.py --output /tmp/p.html
    python scripts/generate_portfolio_report.py --publish          # upload to S3

The Raspberry Pi runs the ``--publish`` form (see runOnPi.sh) and is the only
thing that publishes the public page. It deliberately does **not** run
``main.py``: the Cloud Run Job already sends the Discord notifications, checks
the Harvester rungs, and captures the options snapshots, so a second caller
would double every alert and write a duplicate snapshot per symbol per day.

**One-time setup on the Pi.** Its checkout predates the PostgreSQL move, so it
needs, in this order:

  1. ``QUANTCORE_DB_DSN`` in ``.env`` -- positions and the watchlist come from
     the database now, not from portfolio.csv / watchlist.yaml.
  2. A Cloud SQL Auth Proxy running alongside it, so that DSN's host:port
...
from portfolio import watch_list  # noqa: E402
from quantcore.db import ensure_schema  # noqa: E402
...
def create_portfolio_html(portfolio, watchlist):
    ...
    # The one line that could not move verbatim from main.py: this was
    # Path(__file__).parent, which pointed at the repo root when the function
    # lived there and would point at scripts/ here -- generating a second,
...
def save_html_to_s3(html_content):
    ...
    Raises RuntimeError when BUCKET_NAME/BUCKET_KEY are absent. This used to
    ``return None`` and let the caller carry on reporting success, which is how
    the public page could stop updating without anyone noticing (issue #147).
    main() checks the same two variables up front so the failure lands before
    several minutes of price fetching, not after.
    """
    import boto3
    ...
    if not bucket_name or not key:
        raise RuntimeError(
            "BUCKET_NAME and BUCKET_KEY must both be set to publish to S3; "
            "run without --publish to write the report locally instead."
        )
    else:
...
def build_report() -> str:
    """Load John's positions and the shared watchlist, price them, render the HTML."""
    # Make sure the schema is right (creates tables, or verifies them
    # where Flyway owns the DDL -- see QUANTCORE_SCHEMA_MODE).
    ensure_schema()

    # John's positions come from the DB-backed source of truth (positions
    # table). portfolio.csv remains his import file; refresh it via
    # scripts/import_portfolio.py --csv portfolio.csv --owner john.
    my_portfolio = portfolio_module.Portfolio()
    my_portfolio.read_stocks_from_records(get_services().portfolio.list_positions("john"))
    my_portfolio.update_all_prices()
    my_portfolio.add_descriptive_info_to_stocks()
    my_portfolio.update_metrics()

    # The watchlist is global (issue #83) and likewise DB-backed, with
    # deliberately no fallback to watchlist.yaml. main.py alarms on an empty
    # table; here it just means the report's watchlist section is empty, which
    ...
    return create_portfolio_html(my_portfolio, watchlist)


def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Generate the HTML portfolio report (and optionally publish it to S3).",
    )
    parser.add_argument(
        "--output",
        metavar="PATH",
        help="write the HTML here (default: portfolio_report.html in the repo root)",
    )
    parser.add_argument(
        "--publish",
        action="store_true",
        help="upload to S3 using BUCKET_NAME/BUCKET_KEY instead of writing a file",
    )
...
def main(argv=None) -> int:
    ...
    if args.publish:
        # Publishing was requested explicitly, so missing credentials are a
        # failure, not a fallback. The old code returned None here and the
        # daily job carried on reporting success -- the page silently stopped
        # updating and nobody found out for months (issue #147).
        load_dotenv()
        missing = [n for n in ("BUCKET_NAME", "BUCKET_KEY") if not os.environ.get(n)]
        if missing:
            print(
                f"ERROR: --publish needs {' and '.join(missing)} in the environment "
                "(or .env). Refusing to exit 0 on a report nobody published.",
                file=sys.stderr,
            )
            return 1
    ...
    if args.publish:
        url = save_html_to_s3(html_content)
        if not url:
            print("ERROR: the S3 upload did not return a URL.", file=sys.stderr)
            return 1
        print(f"Portfolio report published: {url}")
        return 0

    html_file = Path(args.output) if args.output else REPO_ROOT / "portfolio_report.html"
    html_file.parent.mkdir(parents=True, exist_ok=True)
    html_file.write_text(html_content)
    print(f"Portfolio report written to file://{html_file}")
    return 0
